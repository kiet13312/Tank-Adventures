...
public class TankGameView extends View {
    ...
    void reset(){
        ...
        playerHP=430;playerArmor=170;levelWon=false;levelLost=false;shots.clear();enemy=makeEnemy(level);enemyNext=System.currentTimeMillis()+1200;
    ...
    Enemy makeEnemy(int lv){
        ...
        if(e.boss){e.hp+=220;e.maxHp=e.hp;e.armor+=90;e.damage+=8;e.fireMs-=180;e.speed+=5;}e.name=e.boss?"BOSS":"XE ĐỊCH "+lv;return e;
    }
    ...
    void drawBattle(Canvas c){
        drawWorld(c);txt(c,"MÀN "+level+"/15",25,34,24,true);txt(c,"HP "+playerHP+"   GIÁP "+playerArmor,25,64,22,true);txt(c,"XU "+coins,25,92,20,false);txt(c,"ENEMY "+Math.max(0,enemy.hp),1020,34,24,true);txt(c,enemy.name,1020,62,18,false);txt(c,"NÒNG "+(int)gunAngle+"°",25,120,18,false);
    ...
    void tryWeld(int i){
        if(i==0)return;Part a=parts.get(i);float best=999;int parent=-1;PointF aa=null,bb=null;for(int j=0;j<parts.size();j++)if(j!=i)for(PointF A:connectors(a))for(PointF B:connectors(parts.get(j))){float d=(float)Math.hypot(A.x-B.x,A.y-B.y);if(d<best){best=d;parent=j;aa=A;bb=B;}}
