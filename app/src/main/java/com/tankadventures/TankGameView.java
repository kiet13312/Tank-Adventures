...
public class TankGameView extends View {
    ...
    void reset(){
        ...
        parts.get(7).angle=18;gunAngle=18;makeRelative();
        worldX=420;playerHP=430;playerArmor=170;levelWon=false;levelLost=false;shots.clear();
        enemy=makeEnemy(level);enemyNext=System.currentTimeMillis()+1200;
    ...
    Enemy makeEnemy(int lv){
        Enemy e=new Enemy();e.x=2250+lv*180;e.hp=250+lv*48;e.maxHp=e.hp;e.armor=100+lv*20;
        e.damage=22+lv*2;e.fireMs=Math.max(850,1900-lv*55);e.speed=15+lv*1.6f;e.boss=lv%5==0;
        if(e.boss){e.hp+=220;e.maxHp=e.hp;e.armor+=90;e.damage+=8;e.fireMs-=180;e.speed+=5;}
        e.name=e.boss?"BOSS":("XE ĐỊCH "+lv);return e;
    }
    float terrainY(float x){
        float phase=level*.63f;
        return 470+55*(float)Math.sin(x/260f+phase)+32*(float)Math.sin(x/105f+level)+18*(float)Math.sin(x/53f+2);
    }
    float terrainSlope(float x){return (terrainY(x+2)-terrainY(x-2))/4f;}
    int levelLength(){return 4500+level*260;}

    @Override protected void onDraw(Canvas c){
        scale=Math.min(getWidth()/1280f,getHeight()/720f);ox=(getWidth()-VW*scale)/2f;oy=(getHeight()-VH*scale)/2f;
        ...
        if(buildMode)drawBuilder(c);else{if(!levelWon&&!levelLost)updateBattle(dt,now);drawBattle(c);}
        c.restore();postInvalidateDelayed(16);
    }
    void bg(Canvas c){
        p.setStyle(Paint.Style.FILL);p.setColor(Color.rgb(67,151,196));c.drawRect(0,0,VW,VH,p);
        p.setColor(Color.argb(55,255,255,255));c.drawCircle(170,100,35,p);c.drawCircle(215,88,48,p);c.drawCircle(263,105,32,p);c.drawCircle(850,125,34,p);c.drawCircle(895,115,52,p);c.drawCircle(952,130,30,p);
    }
    ...
    void drawPart(Canvas c,Part a,int i){
        ...
        if(a.weld>=0)txt(c,"HÀN",a.x-17,a.y+55,14,false);
    }

    void drawBattle(Canvas c){
        drawWorld(c);txt(c,"MÀN "+level+"/15",25,34,24,true);txt(c,"HP "+playerHP+"   GIÁP "+playerArmor,25,64,22,true);txt(c,"XU "+coins,25,92,20,false);
        txt(c,"ENEMY "+Math.max(0,enemy.hp),1020,34,24,true);txt(c,enemy.name,1020,62,18,false);txt(c,"NÒNG "+(int)gunAngle+"°",25,120,18,false);
    ...
    void drawWorld(Canvas c){
        bg(c);cameraX=Math.max(0,Math.min(Math.max(0,levelLength()-VW),worldX-500));c.save();c.translate(-cameraX,0);drawTerrain(c);drawFinish(c);drawPlayer(c);if(enemy.hp>0)drawEnemy(c);for(Shot s:shots)s.draw(c);c.restore();
    }
    void drawTerrain(Canvas c){
        float leftEdge=cameraX-100,rightEdge=cameraX+1380;p.setStyle(Paint.Style.FILL);p.setColor(Color.rgb(103,179,83));Path q=new Path();q.moveTo(leftEdge,terrainY(leftEdge));for(float x=leftEdge;x<=rightEdge;x+=20)q.lineTo(x,terrainY(x));q.lineTo(rightEdge,VH);q.lineTo(leftEdge,VH);q.close();c.drawPath(q,p);
        p.setColor(Color.rgb(174,220,111));Path t=new Path();t.moveTo(leftEdge,terrainY(leftEdge)-20);for(float x=leftEdge;x<=rightEdge;x+=20)t.lineTo(x,terrainY(x)-20);t.lineTo(rightEdge,terrainY(rightEdge)-20);t.lineTo(leftEdge,terrainY(leftEdge)-20);t.close();c.drawPath(t,p);
    }
    ...
    void drawPlayer(Canvas c){
        ...
        for(int i=0;i<parts.size();i++){Part a=parts.get(i);if(i==0||a.weld>=0)drawLocal(a,relX[i],relY[i]);}c.restore();
    }
    void drawLocal(Part a,float x,float y){
        p.setStyle(Paint.Style.FILL);p.setColor(partColor(a.type));
        if(a.type==Part.FRAME)c.drawRoundRect(x-130,y-35,x+130,y+35,18,18,p);
        else if(a.type==Part.MOTOR)c.drawRoundRect(x-55,y-30,x+55,y+30,14,14,p);
        ...
        else{p.setColor(Color.rgb(45,45,45));p.setStrokeWidth(17);p.setStrokeCap(Paint.Cap.ROUND);float r=(float)Math.toRadians(gunAngle);c.drawLine(x,y,x+155*(float)Math.cos(r),y-155*(float)Math.sin(r),p);}
    }
    void drawEnemy(Canvas c){
        float x=enemy.x,y=terrainY(x)-35;p.setStyle(Paint.Style.FILL);p.setColor(Color.argb(60,0,0,0));c.drawOval(x-145,y+25,x+145,y+65,p);p.setColor(Color.rgb(55,55,58));c.drawRoundRect(x-145,y-20,x+145,y+55,28,28,p);
        p.setColor(Color.rgb(145,145,145));for(int i=0;i<7;i++)c.drawCircle(x-105+i*35,y+20,13,p);p.setColor(enemy.boss?Color.rgb(115,55,55):Color.rgb(125,75,60));Path h=new Path();h.moveTo(x-120,y-55);h.lineTo(x+100,y-55);h.lineTo(x+125,y+5);h.lineTo(x-105,y+5);h.close();c.drawPath(h,p);
        p.setColor(enemy.boss?Color.rgb(75,45,45):Color.rgb(95,60,55));c.drawRoundRect(x-50,y-108,x+60,y-50,22,22,p);p.setColor(Color.rgb(45,45,45));p.setStrokeWidth(17);p.setStrokeCap(Paint.Cap.ROUND);c.drawLine(x+12,y-78,x+145,y-78,p);
        p.setColor(Color.rgb(190,190,175));c.drawCircle(x+112,y-15,22,p);p.setColor(Color.argb(160,0,0,0));c.drawRect(x-120,y-150,x+120,y-136,p);p.setColor(Color.rgb(70,220,85));c.drawRect(x-120,y-150,x-120+240*Math.max(0,enemy.hp)/(float)enemy.maxHp,y-136,p);
    }

    void updateBattle(float dt,long now){
        float speed=135+count(Part.MOTOR)*55;
        if(right)worldX=Math.min(levelLength()-250,worldX+speed*dt);if(left)worldX=Math.max(120,worldX-speed*dt);
        if(up)gunAngle=Math.min(78,gunAngle+55*dt);if(down)gunAngle=Math.max(5,gunAngle-55*dt);
        for(Part a:parts)if(a.type==Part.GUN)a.angle=gunAngle;
        if(enemy.hp>0){float target=worldX+720;if(enemy.x>target)enemy.x-=enemy.speed*dt;else if(enemy.x<worldX+500)enemy.x+=enemy.speed*dt;}
        for(Shot s:shots){if(s.dead)continue;s.vy+=760*dt;s.x+=s.vx*dt;s.y+=s.vy*dt;s.life-=dt;
            if(s.y>terrainY(s.x)-5){s.y=terrainY(s.x)-5;s.dead=true;continue;}if(s.life<=0){s.dead=true;continue;}
        ...
        if(now>enemyNext&&enemy.hp>0&&playerHP>0){float ex=enemy.x-125,ey=terrainY(enemy.x)-110,tx=worldX,ty=terrainY(worldX)-90;float a=(float)Math.atan2(ty-ey,tx-ex);shots.add(new Shot(ex,ey,a,590,enemy.damage,true));enemyNext=now+enemy.fireMs;}
        Iterator<Shot>it=shots.iterator();while(it.hasNext())if(it.next().dead)it.remove();
        if(enemy.hp<=0&&!levelWon){levelWon=true;coins+=100;prefs.edit().putInt("coins",coins).apply();}
    ...
    void fire(){
        if(playerHP<=0||enemy.hp<=0||levelWon||levelLost)return;Part g=first(Part.GUN);if(g==null)return;int gi=parts.indexOf(g);float gx=relX[gi],gy=relY[gi];float slope=(float)Math.toDegrees(Math.atan(terrainSlope(worldX));
        float worldAngle=slope-gunAngle;float rad=(float)Math.toRadians(worldAngle);float sx=worldX+gx+(float)Math.cos(rad)*155;float sy=terrainY(worldX)-98+gy-(float)Math.sin(Math.toRadians(gunAngle))*155;shots.add(new Shot(sx,sy,rad,650+count(Part.MOTOR)*30,32,false));
    }

    boolean touchBuilder(MotionEvent e,float x,float y){
        int a=e.getActionMasked();if(a==MotionEvent.ACTION_DOWN){
            if(x>895&&y>590&&x<1080){makeRelative();buildMode=false;return true;}if(x>1085&&y>590){reset();return true;}
            if(x>895&&x<1240&&y>140&&y<575){int t=paletteType(y);if(t>=0){Part z=new Part(t,x-300,y);z.weld=-1;parts.add(z);drag=parts.size()-1;msg="Đang kéo "+name(t);return true;}}
            for(int i=parts.size()-1;i>=0;i--){Part z=parts.get(i);if(Math.hypot(x-z.x,y-z.y)<72){drag=i;return true;}}
        }
        if((a==MotionEvent.ACTION_MOVE||a==MotionEvent.ACTION_UP)&&drag>=0){Part z=parts.get(drag);z.x=x;z.y=y;if(a==MotionEvent.ACTION_UP){tryWeld(drag);drag=-1;}invalidate();return true;}return true;
    }
    ...
    void tryWeld(int i){
        if(i==0)return;Part a=parts.get(i);float best=999;int parent=-1;PointF aa=null,bb=null;
        for(int j=0;j<parts.size();j++)if(j!=i)for(PointF A:connectors(a))for(PointF B:connectors(parts.get(j))){float d=(float)Math.hypot(A.x-B.x,A.y-B.y);if(d<best){best=d;parent=j;aa=A;bb=B;}}
        if(best<55){a.x+=bb.x-aa.x;a.y+=bb.y-aa.y;a.weld=parent;if(a.type==Part.GUN&&parts.get(parent).type==Part.HINGE)a.angle=gunAngle;msg="ĐÃ HÀN: "+name(a.type)+" vào "+name(parts.get(parent).type);}else msg="Chưa khớp mấu nối";
    }

    boolean touchBattle(MotionEvent e,float x,float y){
        int a=e.getActionMasked();if(a==MotionEvent.ACTION_DOWN||a==MotionEvent.ACTION_MOVE){left=x<120&&y>570;right=x>=120&&x<225&&y>570;up=x>=225&&x<325&&y>570;down=x>=325&&x<425&&y>570;
            if(a==MotionEvent.ACTION_DOWN&&x>1075&&y>570){fire();return true;}if(a==MotionEvent.ACTION_DOWN&&x>870&&x<1070&&y>570){buildMode=true;return true;}
            if(a==MotionEvent.ACTION_DOWN&&levelWon&&level<15&&x>480&&x<800&&y>380&&y<480){nextLevel();return true;}if(a==MotionEvent.ACTION_DOWN&&levelLost&&x>480&&x<800&&y>380&&y<480){retryLevel();return true;}return true;}
        left=right=up=down=false;return true;
    }
    ...
    class Enemy{float x,speed;int hp,maxHp,armor,damage,fireMs;boolean boss;String name="";boolean hit(float px,float py){float gy=terrainY(x)-35;return px>x-130&&px<x+130&&py>gy-115&&py<gy+60;}}
}